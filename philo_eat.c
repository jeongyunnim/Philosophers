...
/*   Updated: 2023/02/06 22:00:07 by jeseo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_down_forks_even(t_philo *shared, int left_fork, int right_fork)
{
	shared->fork[left_fork] = 0;
	shared->fork[right_fork] = 0;
	pthread_mutex_unlock(&shared->fork_mutex[left_fork]);
...
void	put_down_forks_odd(t_philo *shared, int left_fork, int right_fork)
{
	shared->fork[left_fork] = 0;
	shared->fork[right_fork] = 0;
	pthread_mutex_unlock(&shared->fork_mutex[right_fork]);
...
int	eating_spaghetti(t_philo *shared, int num, int left_fork, int right_fork)
{
	...
	if (num % 2 == 1)
		put_down_forks_even(shared, left_fork, right_fork);
	else
		put_down_forks_odd(shared, left_fork, right_fork);
	//put_down_forks(shared, left_fork, right_fork);
	return (0);
}
